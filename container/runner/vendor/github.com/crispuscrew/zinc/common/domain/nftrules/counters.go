// Counters reads back what a ruleset has seen. It is the inverse of Render and lives beside it
// for the same reason: a counter's meaning cannot differ between the runtime that renders the
// rule and the one that reads it.
package nftrules
...
// "counters") and what it has seen. A counter is created with the namespace it lives in, so it
// reads "since this launch": stopping the app takes the namespace and the numbers with it.
